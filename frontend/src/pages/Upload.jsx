      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <i className={`ti ${danger ? "ti-alert-triangle" : "ti-help-circle"}`}
            style={{ fontSize: 20, color: danger ? "var(--red)" : "var(--amber)" }}
          />
          <span style={{ fontSize: 15, fontWeight: 600, color: "var(--text-primary)" }}>{title}</span>
        </div>
        <p style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.65, marginBottom: 22 }}>
          {body}
        </p>
  ...
  return (
    ...
    >
      <i className={`ti ${fileIcon(doc.filename)}`}
        style={{ fontSize: 16, color: "var(--accent-light)", flexShrink: 0 }}
      />
      <span style={{
...
export default function Upload() {
  ...
  const [confirmDelete, setConfirmDelete]       = useState(null); // filename string
  ...
  const uploadFile = useCallback(async (file) => {
    ...
    try {
      ...
      // Refresh document list so it appears immediately
      await refreshDocs();
    } catch (err) {
      clearInterval(ticker);
      setProgress(0);
    ...
    <div className="upload-wrap">
      ...
      {confirmClearAll && (
        <ConfirmModal
          ...
          body="This will permanently delete all uploaded documents, their embeddings, the knowledge graph, and conversation history. The AI will have no context until you upload new documents."
      ...
      <div className="upload-log">
        <div className="upload-log-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button
            ...
            title="Refresh list"
          >
            <i className={`ti ${docsLoading ? "ti-loader-2" : "ti-refresh"}`}
              style={docsLoading ? { animation: "spin 1s linear infinite" } : {}}
            />
          </button>
        </div>
        ...
        {docs.length > 0 && (
          <div style={{
            ...
            <strong style={{ color: "var(--text-secondary)" }}>Tip:</strong> Fewer, focused documents
            reduce retrieval noise and lower hallucination risk. Remove documents that are no longer
            relevant to your current question set.
          </div>
        )}
      </div>
      ...
      {log.length > 0 && (
        <div className="upload-log">
          <div className="upload-log-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span>
              <i className="ti ti-terminal" style={{ marginRight: 6, verticalAlign: "middle" }} />
              Activity log
            </span>
            <button
              className="btn"
              style={{ padding: "2px 8px", fontSize: 11, gap: 4, color: "var(--red)", borderColor: "var(--red-dim)" }}
              onClick={() => setLog([])}
              title="Clear log"
            >
              <i className="ti ti-x" />
