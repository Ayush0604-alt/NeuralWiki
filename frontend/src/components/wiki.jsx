...
// ── Markdown renderer (reuses same inline parser pattern from ChatBox) ────────
function WikiContent({ text }) {
  ...
  while (i < lines.length) {
        ...
        elements.push(
          <div key={`t-${i}`} style={{ overflowX: "auto", margin: "12px 0" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr>{rows[0].map((cell, j) => (
                  <th key={j} style={{ padding: "8px 14px", background: "var(--bg-raised)", border: "1px solid var(--border)", color: "var(--text-primary)", fontWeight: 600, textAlign: "left", fontSize: 12, letterSpacing: "0.3px" }}>{inline(cell)}</th>
                ))}</tr>
              </thead>
              <tbody>
                {rows.slice(1).map((row, ri) => (
                  <tr key={ri} style={{ background: ri % 2 === 0 ? "transparent" : "var(--bg-surface)" }}>
                    {row.map((cell, ci) => (
                      <td key={ci} style={{ padding: "8px 14px", border: "1px solid var(--border)", color: "var(--text-secondary)", fontSize: 13 }}>{inline(cell)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
    ...
    if (line.startsWith("### ")) {
      elements.push(
        <div key={`h3-${i}`} style={{ fontSize: 13, fontWeight: 600, color: "var(--accent-light)", margin: "14px 0 5px" }}>
          {inline(line.slice(4))}
        </div>
      );
      i++; continue;
        ...
        <div key={`ol-${i}`} style={{ margin: "8px 0", display: "flex", flexDirection: "column", gap: 6 }}>
          {items.map(({ num, content }, j) => (
            <div key={j} style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 13 }}>
              ...
              <span style={{ color: "var(--text-secondary)", lineHeight: 1.65 }}>{inline(content)}</span>
            </div>
          ))}
        </div>
      );
    ...
    if (/^[-*•]\s/.test(line)) {
      ...
      elements.push(
        <div key={`ul-${i}`} style={{ margin: "6px 0", display: "flex", flexDirection: "column", gap: 5 }}>
          {items.map((item, j) => (
            <div key={j} style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 13 }}>
              <span style={{ color: "var(--accent-light)", marginTop: 5, fontSize: 7, flexShrink: 0 }}>●</span>
              <span style={{ color: "var(--text-secondary)", lineHeight: 1.65 }}>{inline(item)}</span>
            </div>
          ))}
        </div>
      );
    ...
    // Paragraph
    elements.push(
      <p key={`p-${i}`} style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.75, margin: "3px 0" }}>
        {inline(line)}
      </p>
    );
    i++;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        padding: "18px 28px 14px", borderBottom: "1px solid var(--border-subtle)", flexShrink: 0,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button className="btn" onClick={onBack} style={{ padding: "6px 10px" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2, display: "flex", gap: 10 }}>
              ...
              <span><i className="ti ti-align-left" style={{ marginRight: 4 }} />{wordCount.toLocaleString()} words in source</span>
            </div>
          </div>
        </div>
        <button
          className="btn"
          onClick={() => onRegenerate(page.filename)}
          disabled={regenerating === page.filename}
          title="Regenerate wiki page using AI"
        >
          <i className={`ti ${regenerating === page.filename ? "ti-loader-2" : "ti-refresh"}`}
             style={regenerating === page.filename ? { animation: "spin 1s linear infinite" } : {}} />
      ...
      <div style={{ padding: "20px 28px 16px", borderBottom: "1px solid var(--border-subtle)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            ...
            <div className="page-subtitle">AI-generated knowledge pages for each document</div>
          </div>
          {missing.length > 0 && (
            <button className="btn btn-primary" onClick={onGenerateAll} disabled={!!regenerating} style={{ gap: 7 }}>
              <i className="ti ti-wand" />
              Generate {missing.length} missing page{missing.length !== 1 ? "s" : ""}
            </button>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 28px", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
              <span style={{
                ...
                <i className="ti ti-check" style={{ marginRight: 3 }} />wiki ready
              </span>
              <button
                className="btn"
                style={{ padding: "3px 10px", fontSize: 11 }}
                onClick={e => { e.stopPropagation(); onRegenerate(p.filename); }}
                disabled={regenerating === p.filename}
              >
                <i className={`ti ${regenerating === p.filename ? "ti-loader-2" : "ti-refresh"}`}
                   style={regenerating === p.filename ? { animation: "spin 1s linear infinite" } : {}} />
                {regenerating === p.filename ? "…" : "Regenerate"}
              </button>
            </div>
          </div>
        ))}

        {/* Docs without wiki pages */}
        {missing.map(d => (
          <div key={d.filename} style={{
            ...
            <div style={{
              ...
              <i className={`ti ${fileIcon(d.filename)}`} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 2 }}>
            ...
            <button
              ...
              onClick={() => onRegenerate(d.filename)}
              disabled={regenerating === d.filename}
              style={{ flexShrink: 0 }}
            >
              <i className={`ti ${regenerating === d.filename ? "ti-loader-2" : "ti-wand"}`}
                 style={regenerating === d.filename ? { animation: "spin 1s linear infinite" } : {}} />
        ...
        {pages.length === 0 && missing.length === 0 && !loading && (
          <div className="empty-state">
            ...
            <p>Upload documents to automatically generate wiki pages, or upload a document first.</p>
          </div>
        )}
      </div>
    </div>
...
export default function Wiki() {
  ...
  const regenerate = async (filename) => {
    ...
    try {
      ...
      if (selectedPage?.filename === filename) {
        setSelectedPage(data.page);
      }
    } catch (e) { setError(e.message); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      {error && (
        <div style={{
          margin: "0 28px", marginTop: 12, padding: "10px 14px",
